  ...
  <main class="main-container main-container--fullheight commands">
    <div class="container">
      <div ref="terminal" class="terminal" @click="focusInput">
        <div
          v-for="({ type, time, message }, i) in log"
          :key="i"
          class="terminal-message"
        >
          <span v-if="timestamps" class="terminal-message__time timestamp"
            >[{{ time }}]</span
          >
          <span
            class="terminal-message__sign"
            :class="`terminal-message__sign--${type}`"
            v-text="type === 'out' ? '>' : '<'"
          ></span>
        ...
        <div class="terminal__input-wrapper">
          <span
            v-tooltip="$t('commands-send')"
            class="terminal-message__sign sign-input"
            @click="sendCommand"
            >></span
          >
          <input
          ...
          />
          <input
            v-model="autocompleteSuggestion"
            type="text"
            spellcheck="false"
            class="terminal__input terminal__input--autocomplete"
          />
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    commandsNames() {
      return this.commands
        .map((command) => command.command.split(" ")[0])
        .sort();
    },
    uiCommands() {
      return [
        ...
        { command: "clear", description: this.$t("terminal-command-clear") },
        {
          ...
          description: this.$t("terminal-command-clear-history"),
        },
      ];
    },
    uiCommandsNames() {
      return this.uiCommands
        .map((uiCommand) => uiCommand.command.split(" ")[0])
        .sort();
    },
    allCommands() {
    ...
    allCommandsParameters() {
      return (
        this.allCommands
          ...
          .reduce(
            (commandParameters, { command, params }) => (
              (commandParameters[command] = params), commandParameters
            ),
            {}
          )
      );
    ...
    autocompleteSuggestion() {
      if (this.suggestedCommand)
        return (
          ...
          this.suggestedCommand.substr(this.command.length)
        );

      if (this.selectedCommand) {
        if (!this.suggestedParameters || !this.suggestedParameters.length)
          return;

        if (this.suggestedParameterValue) {
          return [
            this.command.replace(/./g, " ") +
              this.suggestedParameterValue.slice(
        ...
        const remainingParameters = this.suggestedParameters.slice(
          this.currentParameterValue.length
            ? this.currentParameterIndex
            : this.currentParameterIndex - 1
        );
        if (!remainingParameters.length && this.currentParameter)
          remainingParameters.push(this.currentParameter);
    ...
    suggestedCommand() {
      ...
      return this.allCommandsNames.find((command) =>
        command.startsWith(this.command)
      );
    },
    suggestedParameters() {
      if (
        this.selectedCommand &&
        this.allCommandsParameters[this.selectedCommand]
      )
        return this.allCommandsParameters[this.selectedCommand];
    ...
    currentParameter() {
      ...
      const currentParameter =
        this.suggestedParameters[this.currentParameterIndex - 1];
      if (currentParameter) return currentParameter;

      const lastParameter =
        this.suggestedParameters[this.suggestedParameters.length - 1];
      if (lastParameter.substr(-2, 1) === "s") return lastParameter;
    },
    currentParameterValue() {
      if (
        this.currentParameter &&
        this.currentParameter.substr(-2, 1) === "s"
      ) {
        const currentParameterValue = this.command
          .split(" ")
          [this.currentParameterIndex].split(",");
    ...
    suggestedParameterValue() {
      if (!this.currentParameterValue || !this.currentParameterValue.length)
        return;
      ...
      switch (this.currentParameter.toLowerCase()) {
        ...
        case "<targetbot>":
          ...
          const suggestedBot = [
            ...this.$store.getters["bots/bots"].map((bot) => bot.name),
            "ASF",
          ...
          return [
            ...
            name
              .toLowerCase()
              .startsWith(this.currentParameterValue.toLowerCase())
          );
        case "<command>":
          return this.allCommandsNames.find((name) =>
            name.startsWith(this.currentParameterValue)
          );
        case "<modes>":
          ...
          if (this.selectedCommand === "redeem^") {
            return ["FD", "FF", "FKMD", "SD", "SF", "SI", "SKMG", "V"].find(
              (name) =>
                name
                  .toLowerCase()
                  .startsWith(this.currentParameterValue.toLowerCase())
        ...
        case "<settings>":
          ...
          return ["Private", "FriendsOnly", "Public"].find((name) =>
            name
              .toLowerCase()
              .startsWith(this.currentParameterValue.toLowerCase())
          );
    ...
    selectedCommand() {
      if (!this.command) return;
      return this.allCommandsNames.find(
        (command) => command === this.command.split(" ")[0]
      );
    },
  },
  watch: {
    log() {
      this.$nextTick(() => {
        this.$refs.terminal.scrollTop = Math.max(
          0,
          this.$refs.terminal.scrollHeight - this.$refs.terminal.clientHeight
        );
      });
    },
  },
  ...
  mounted() {
    this.log.push({
      ...
      message: this.$t("commands-default"),
    });
  ...
  methods: {
    ...
    async executeCommand(commandToExecute) {
      switch (commandToExecute.split(" ")[0]) {
        case "commands":
          return this.$t("terminal-available-commands", {
            commands: this.commandsNames.join(", "),
            uiCommands: this.uiCommandsNames.join(", "),
          });
        case "help":
          if (commandToExecute.split(" ")[1])
            return this.commandHelp(commandToExecute.split(" ")[1]);
    ...
    commandHelp(command) {
      const foundCommand = this.allCommands.find(
        (allCommand) => allCommand.command.split(" ")[0] === command
      );
    ...
    autocomplete() {
      if (!this.selectedCommand)
        this.command = this.suggestedCommand || this.command;

      if (this.selectedCommand && this.suggestedParameterValue) {
        const splitCommand = this.command.split(" ");
        const splitCurrentParameter =
          splitCommand[splitCommand.length - 1].split(",");

        this.command = [
          ...splitCommand.slice(0, -1),
          [
            ...splitCurrentParameter.slice(0, -1),
            this.suggestedParameterValue,
          ].join(","),
        ].join(" ");
        ...
        if (tabPressTime - this.lastTabPressTime <= 500)
    ...
    parseCommandsHTML(commandsWikiRaw) {
      ...
      const commandsTableHTML =
        virtualDOM.querySelector("h2 > a").parentElement.nextElementSibling;
    ...
    async fetchCommands() {
      ...
      storage.set(`cache:asf-commands:${locale}`, {
        timestamp: Date.now(),
        commands,
      });
