  ...
  <main v-if="bot" class="main-container">
    ...
    <div v-else class="form-item">
      <div class="form-item__token">
        <input
          class="form-item__input form-item__input-token"
          type="text"
          :value="token"
          readonly
        />
        <div class="form-item__buttons form-item__buttons--column">
          <button
            v-tooltip.left="$t('2fa-token-refresh')"
          ...
          <button
            v-tooltip.left="$t('2fa-token-copy')"
            class="button button--helper"
            @click="copyToken"
          >
      ...
      <div
        class="form-item__buttons form-item__buttons--center form-item__buttons--column"
      >
      ...
      <div class="form-item__delete">
        <a @click="$router.push({ name: 'bot-2fa-delete' })">{{
          $t("2fa-delete", { name: bot.viewableName })
        }}</a>
      </div>
    </div>
  </main>
  ...
  methods: {
    async onImport() {
      ...
      try {
        ...
        const response = await this.$http.post(
          `bot/${bot}/twoFactorAuthentication`,
          JSON.parse(maFileContent)
        );
    ...
    async acceptConfirmations() {
      ...
      try {
        const bot = this.bot.name;
        const response = await this.$http.post(
          `bot/${bot}/twoFactorAuthentication/confirmations`,
          { accept: true }
        );
    ...
    async refreshToken() {
      ...
      try {
        ...
        const response = await this.$http.get(
          `bot/${bot}/twoFactorAuthentication/token`
        );
