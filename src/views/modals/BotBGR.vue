<template>
  <main v-if="bot" class="main-container">
    ...
    <div
      v-if="!loading && state === 'input' && bot.bgrCount !== 0"
      class="bgr__info"
    >
    ...
    <keep-alive>
      ...
      <BgrCheck
        v-if="state === 'check'"
        :keys="keys"
        :title="$t('bgr-check', { n: foundKeysCount })"
        :bot="bot"
      ...
      <BgrSummary
        v-if="state === 'summary'"
        :keys="summaryKeys"
        :title="$t('bgr-summary-success', { n: addedKeysCount })"
        @back="$parent.back()"
      ></BgrSummary>
      <BgrSummary
        v-if="state === 'usedKeys'"
        :keys="usedKeys"
        :title="$t('bgr-used-keys')"
        @back="state = 'input'"
      ></BgrSummary>
      <BgrSummary
        v-if="state === 'unusedKeys'"
        :keys="unusedKeys"
        :title="$t('bgr-unused-keys')"
        @back="state = 'input'"
      ></BgrSummary>
    </keep-alive>
  </main>
...
export default {
  ...
  methods: {
    async loadBGR() {
      return (
        await this.$http.get(`bot/${this.bot.name}/gamesToRedeemInBackground`)
      )[this.bot.name];
    },
    onCheck(keys) {
      this.keys = keys;
    ...
    async onConfirm() {
      ...
      try {
        const activatedKeys = await this.$http.post(
          `bot/${this.bot.name}/gamesToRedeemInBackground`,
          { gamesToRedeemInBackground: this.keys }
        );
        this.state = "summary";
