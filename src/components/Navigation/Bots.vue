<template>
  <div class="bots-navigation">
    <!-- Existing bot status cards -->
    <div class="bot-cards">
      <div
        v-for="(type, i) in botTypes"
        :key="i"
        class="bot-card"
        :class="[
          `status--${type}`,
          { 'bot-card--selected': selectedBots.includes(type) }
        ]"
...
export default {
  ...
  components: {
    FitText,
    BotCard // Register the component
  },
  data() {
    return {
  ...
  methods: {
    recalculateText() {
      ...
      if (this.transitioning)
        requestAnimationFrame(() => this.recalculateText());
    },
    onTransitionStart() {
