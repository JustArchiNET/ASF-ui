  ...
  <div class="form-item">
    ...
    <div class="form-item__value">
      <div class="input-option__field input-option__field--three">
        ...
        >
          <option
            v-for="(enumValue, name) in schema.value.values"
            :key="name"
            :value="enumValue"
          >
            {{ name }}
...
export default {
  ...
  methods: {
    ...
    resolveValue(value) {
      ...
      return Object.keys(this.schema.value.values).find(
        (key) => this.schema.value.values[key] === value
      );
    },
    addElement() {
      if (
        (!this.elementValue && this.elementValue !== 0) ||
        (!this.elementKey && this.elementKey !== 0)
      )
        return;
