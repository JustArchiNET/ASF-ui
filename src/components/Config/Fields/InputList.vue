  ...
  <div class="form-item">
    ...
    <div class="form-item__value">
      <select
        ...
        @change="addElement($event.target.value)"
      >
        <option :value="null" disabled selected hidden>
          {{ $t("input-select-enum-value") }}
        </option>
        <option
          v-for="(enumValue, name) in enumValues"
          v-show="!value.includes(enumValue)"
          :key="name"
          :value="enumValue"
        >
          {{ name }}
      ...
      <div class="input-option__items">
        <button
          ...
          :key="index"
          class="button input-option__item"
          @click.prevent="removeElementAtIndex(index)"
        >
          {{ resolveOption(item) }}
        </button>
      </div>
...
export default {
  ...
  methods: {
    addElement(input) {
      const parsedInput =
        typeof input !== typeof 0 ? parseInt(input, 10) : input;
    ...
    resolveOption(value) {
      return Object.keys(this.enumValues).find(
        (key) => this.enumValues[key] === value
      );
    },
  },
};
