  ...
  <div class="form-item input-option">
    ...
    <div class="form-item__value">
      <select
        ...
        @change="addFlag($event.target.value)"
      >
        <option :value="null" disabled selected hidden>
          {{ $t("input-select-enum-value") }}
        </option>
        <option
          v-for="(enumValue, name) in flags"
          v-show="enumValue === 0 || !((value & enumValue) === enumValue)"
          :key="name"
          :value="enumValue"
        >
          {{ name }}
      ...
      <div class="input-option__items">
        <button
          v-for="enumValue in getSelectedFlagValues()"
          :key="enumValue"
          class="button input-option__item"
          @click.prevent="removeFlag(enumValue)"
        >
