<template>
  <div class="config-editor">
    <template v-if="nonEmptyCategories">
      ...
      >
        <component
          :is="componentFromField(field)"
          v-for="field in categoryFields(category.name)"
          :key="field.param"
          class="form-item--config"
          :schema="field"
          :currentValue="model[field.paramName]"
          @update="updateModel"
        ></component>
      </ConfigCategory>

      <ConfigCategory
        v-if="uncategorizedFields.length"
        key="Other"
        :name="$t('other')"
      >
        <component
  ...
  computed: {
    nonEmptyCategories() {
      ...
      return this.categories.filter(
        (category) => this.categoryFields(category.name).length
      );
    },
    uncategorizedFields() {
      if (!this.categories) return this.fields;

      const categorizedFields = this.categories
        .map((category) => category.fields)
        .reduce(
          (categorizedFields, categoryFields) => [
            ...categorizedFields,
            ...categoryFields,
          ],
          []
      ...
      return this.fields.filter(
        (field) => !categorizedFields.includes(field.param)
      );
    },
    categoryFields() {
      return (categoryName) => {
        ...
        if (!category) return [];
        return this.getFields(category.fields).sort(
          (a, b) =>
            category.fields.indexOf(a.paramName) -
            category.fields.indexOf(b.paramName)
        );
      };
    },
    isValid() {
  ...
  methods: {
    componentFromField(field) {
      switch (field.type) {
        ...
        case "hashSet":
        case "list":
          if (["enum"].includes(field.values.type))
            return field.type === "list" ? InputList : InputSet;
          if (
            ["byte", "uint16", "uint32", "uint64", "string"].includes(
              field.values.type
            )
          )
            return InputTag;
    ...
    updateModel(value, field) {
      const fieldSchema = this.fields.find(
        (fieldSchema) => fieldSchema.paramName === field
      );
      const defaultCheck =
        this.deleteDefaultValues &&
        fieldSchema &&
        typeof fieldSchema.defaultValue !== "undefined" &&
        this.isDefault(value, fieldSchema);

      if (defaultCheck) {
        delete this.model[field];
      } else if (
        this.model[field] &&
        this.model[field].constructor === Array &&
        fieldSchema.type !== "dictionary"
      ) {
        // eslint-disable-next-line vue/no-mutating-props
        this.model[field] = value.sort((a, b) => a - b);
      } else if (
        this.model[field] &&
        this.model[field].constructor === Object &&
        fieldSchema.type !== "dictionary"
      ) {
        // eslint-disable-next-line vue/no-mutating-props
        this.model[field] = Object.keys(value).sort(
          (a, b) => value[a] - value[b]
        );
      } else {
        // eslint-disable-next-line vue/no-mutating-props
    ...
    computeLabelWidth() {
      ...
      this.$nextTick(() => {
        const labelWidth = Math.max(
          ...Array.from(this.$el.querySelectorAll(".form-item__label")).map(
            (el) => Math.ceil(parseFloat(getComputedStyle(el).width))
          )
        );
