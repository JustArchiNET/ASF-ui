<template>
  <div class="bots">
    ...
    <router-link v-slot="{ navigate }" custom :to="{ name: 'bot-create' }">
      <div
        class="bot-placeholder status--disabled"
        :class="{ 'bot-placeholder--big': selectedButtonsCount > 2 }"
        @click="navigate"
      >
        <div class="bot-placeholder__button bot-placeholder__button--add">
          <FontAwesomeIcon
            icon="plus"
            class="bot-placeholder__icon"
          ></FontAwesomeIcon>
...
export default {
  ...
  computed: {
    ...
    visibleBots() {
      const visibleBots = this.bots
        .filter((bot) => bot.isVisible(this.selectedBots))
        .sort(this.sortDefault());
      if (this.orderDisabledBotsLast)
        return visibleBots.sort(this.sortByStatus());
      return visibleBots;
    },
  },
  methods: {
