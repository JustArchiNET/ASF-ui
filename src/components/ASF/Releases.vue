  ...
  <div class="releases">
    ...
    <div v-for="(release, i) in releases" v-else :key="i" class="release">
      <div class="release__title">
        ...
        <span
          class="release__badge"
          :class="[
            release.stable
              ? 'release__badge--stable'
              : 'release__badge--prerelease',
          ]"
          >{{ release.stable ? $t("stable") : $t("pre-release") }}</span
        >
        <span
          v-if="
            updatesEnabled &&
            ...
            isNewer(release.version)
          "
          class="release__badge release__badge--install"
          @click="update"
          >{{ $t("releases-install") }}</span
        >
        <span class="release__time">{{
          getTimeText(release.publishedAt)
        }}</span>
      </div>
      ...
      <a
        class="release__changelog-link"
        target="_blank"
        rel="noreferrer noopener"
        :href="`https://github.com/JustArchiNET/ArchiSteamFarm/releases/tag/${release.version}`"
        >{{ $t("changelog-full") }}</a
      >
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    statusText() {
      ...
      if (!this.loading && !this.releases.length)
        return this.$t("releases-not-found");
    },
  },
  async created() {
    try {
  ...
  methods: {
    ...
    isLatestForUpdateChannel(i) {
      if (this.updateChannel === UPDATECHANNEL.EXPERIMENTAL && i === 0)
        return true;
    ...
    async update() {
      ...
      const notification = this.$snotify.info(
        this.$t("update-trying"),
        this.$t("info")
      );
      ...
      try {
        ...
        if (localVersion === remoteVersion)
          this.$info(this.$t("update-is-up-to-date"));
    ...
    async fetchReleases() {
      ...
      if (!releases[0] || !releases[0].stable)
        releases.push(await this.fetchRelease("latest")); // If the latest releases is not stable, fetch latest stable release
      if (
        !releases.some((release) => release.version === this.version) &&
        this.version <= releases[0].version
      )
        releases.push(await this.fetchRelease(this.version)); // If current version is neither latest nor latest stable but not newer than latest, fetch it
      ...
      return releases
        ...
        ) // Clean the list in case any of the fetches failed, remove any duplicates
