<template>
  <div class="terminal-container">
    ...
    <div ref="terminal" class="terminal">
      ...
      <div
        v-for="{ type, message, index } in log"
        :key="index"
        class="terminal-message terminal-message--truncated"
      >
        <span class="terminal-message__content">
          <span
            v-if="selectedLayout.includes('time')"
            class="terminal-message__time"
            >[{{ handleTime(message.time) }}]</span
          >
          <span
            v-if="selectedLayout.includes('process')"
            class="terminal-message__process"
            >{{ message.process }}</span
          >
          <span
            v-if="selectedLayout.includes('level')"
            class="terminal-message__level"
            :class="`terminal-message__level--${message.level.toLowerCase()}`"
            >{{ message.level }}</span
          >
          <span
            v-if="selectedLayout.includes('logger')"
            class="terminal-message__logger"
            >{{ message.logger }}</span
          >
          <span>></span>
...
export default {
  ...
  watch: {
    log() {
      if (
        ...
        this.$refs.terminal.scrollHeight - this.$refs.terminal.clientHeight - 20
      )
        return;
  ...
  created() {
    const websocketURL = `${window.__BASE_URL__.replace(
      /^http/,
      "ws"
    )}api/nlog${
  ...
  methods: {
    ...
    onClose(event) {
      ...
      if (event.code !== 1000)
        this.$error(`Websocket error! Error code: ${event.code}`);
    },
    async onDownload() {
    ...
    async onLoadPrevious() {
      ...
      try {
        if (!this.inizialized) {
          const initialCall = await this.$http.get(
            `nlog/file?count=${this.count}`
          );
        ...
        const previous = await this.$http.get(
          `nlog/file?count=${this.count}&lastAt=${this.lastAt}`
        );
        if (this.lastAt > 0) this.lastAt -= this.count;

        const previousLog = previous.Content.map((x) => ({
          type: "in",
          message: this.parseMessage(x),
        }));
    ...
    handleTime(time) {
      ...
      if (this.logTimestamp === "timeOnlyEu")
        return time.toLocaleTimeString("de-DE", timeOnlyOptions);
      if (this.logTimestamp === "timeOnlyLocale")
        return time.toLocaleTimeString();
      if (this.logTimestamp === "timeOnlyUs")
        return time.toLocaleTimeString("en-US", timeOnlyOptions);
      if (this.logTimestamp === "timeDateEu")
        return time.toLocaleString("de-DE", timeDateOptions);
      if (this.logTimestamp === "timeDateLocale") return time.toLocaleString();
      if (this.logTimestamp === "timeDateUs")
        return time.toLocaleString("en-US", timeDateOptions);
    },
  },
};
</script>
