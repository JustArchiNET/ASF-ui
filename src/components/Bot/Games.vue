<template>
  <div
    v-if="bot.games.length && botsFarmingCount !== 0"
    :class="[gameName ? 'bot-games-text' : 'bot-games']"
  >
    ...
    <template v-else>
      <div
        v-for="(game, i) in bot.games"
        :key="i"
        v-tooltip="game.GameName"
        class="bot-game"
        :class="[game.farming ? 'status--farming' : 'status--disabled']"
      >
        <a
          target="_blank"
