  ...
  <footer class="footer">
    <div class="footer__links">
      <FooterLink
        ...
        prefix="fab"
        icon="github"
        to="https://github.com/JustArchiNET"
      ></FooterLink>
      <FooterLink
        :name="$t('discussions')"
        icon="comments"
        to="https://github.com/JustArchiNET/ArchiSteamFarm/discussions"
      ></FooterLink>
      <FooterLink
        :name="$t('wiki')"
        icon="book-open"
    ...
    <div v-if="authenticated" class="footer__statistic">
      <FontAwesomeIcon
        v-if="newReleaseAvailable"
        v-tooltip="$t('update-available')"
        class="footer__statistic-notify"
        icon="exclamation"
        size="sm"
        @click="redirectToReleases"
      ></FontAwesomeIcon>
...
export default {
  ...
  async mounted() {
    await delay(3000);
    if (this.authenticated && this.updatesEnabled && this.canUpdate)
  ...
  methods: {
    async checkForNewRelease() {
      try {
        ...
        if (
          this.newReleaseAvailable &&
          this.notifyRelease &&
          this.$route.name !== "releases"
        ) {
